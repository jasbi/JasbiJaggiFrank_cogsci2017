...
ggplot(mutate(graph_data,exclusion=reorder(exclusion,prop)), aes(x="",y = prop,fill=exclusivity)) + 
  geom_col(width=0.7) +
  geom_linerange(aes(ymax = cih, ymin = cil)) +
  guides(fill=FALSE) +
  theme_few(base_size = 10) + 
  labs(title = "", x = "", y = "") + 
  scale_colour_solarized() + facet_wrap(exclusion~intonation, ncol=6)
```
